...
def step8_submit_to_api(planning, user_id, api_client: ApiClient):
    """
    Sends each course in the planning list to the API.
    ...
    """
    if not planning:
        logger.info(f"No planning data to send to API for user {user_id}.")
        return True

    logger.info(f"Step 8: Sending {len(planning)} courses to API for user {user_id}.")
    ...
    for course in planning:
        ...
        try:
            ...
            logger.error(f"API Error sending course for user {user_id}: {course_payload} | Error: {e}")
            failure_count += 1
        except Exception as e:
            logger.error(f"Unexpected error sending course to API: {course_payload} | Error: {e}")
            failure_count += 1

    logger.info(f"Step 8 Finished: Successfully sent {success_count}/{len(planning)} courses for user {user_id}.")
